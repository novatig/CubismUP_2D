...
void PutObjectsOnGrid::towers(Shape * const shape) const
{
  ...
  #pragma omp parallel
  {
    ...
    for (size_t i=0; i < Nblocks; i++)
    {
      ...
      //CHI_MAT & __restrict__  chi = BLOCK[chiInfo[i].blockID]->chi;
      UDEFMAT & __restrict__ udef = OBLOCK[chiInfo[i].blockID]->udef;
      ...
      for(int ix=0; ix<VectorBlock::sizeX; ix++)
      {
        ...
        if ( std::fabs( SDIST(ix,iy).s ) < 3*h )
        {
          const Real uTot = u_s - omega_s*(p[1]-Cy) + udef[iy][ix][0];
          const Real vTot = v_s + omega_s*(p[0]-Cx) + udef[iy][ix][1];
          UDEF(ix,iy).u[0] += uTot; //plus equal in case of
          UDEF(ix,iy).u[1] += vTot; //overlapping objects
        }
