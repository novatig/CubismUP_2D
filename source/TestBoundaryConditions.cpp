...
#pragma omp for schedule(static)
        for (int i=0; i<N; i++)
        {
            ...
            for(int iy=0; iy<FluidBlock::sizeY; ++iy)
                for(int ix=0; ix<FluidBlock::sizeX; ++ix)
                {
                    b(ix,iy).u   = lab(ix       ,iy-offset).tmp; // N
                    b(ix,iy).v   = lab(ix       ,iy+offset).tmp; // S
                    b(ix,iy).p   = lab(ix+offset,iy       ).tmp; // W
                    b(ix,iy).chi = lab(ix-offset,iy       ).tmp; // E
                }
        }
    }
